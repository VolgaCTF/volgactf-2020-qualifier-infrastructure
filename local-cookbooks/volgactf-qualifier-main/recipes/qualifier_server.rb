...
postgres_superuser_pwd = secret.get("postgres:password:#{postgres_superuser}")
...
if node['netdata']['slave']['enabled']
  ...
  netdata_stream 'stream' do
    ...
    configurations(
      'enabled' => 'yes',
      'destination' => node['netdata']['slave']['stream']['destination'],
      'api key' => secret.get("netdata:stream:api_key:#{node['netdata']['slave']['stream']['name']}", required: node['netdata']['slave']['enabled'], prefix_fqdn: false)
    )
  end
